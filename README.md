...
4. Load the Docker image
    ```bash
    docker load -i odb-wr-sender.tar
5. Run the service
    ```bash
    docker compose up -d
6. Compile **odb-wr-sender.sp**
7. If you are going to change the **address|port** then configure **odb-wr-sender.smx** ConVars in cfg/sourcemod/plugin.odb-wr-sender.cfg
